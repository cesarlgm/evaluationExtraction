...
for (page in 1:total_page) {
    ...
    name_line<- str_split(page_lines[name_pos]," ")[[1]]
    if(!is.na(name_pos)){  
      ...
      concepts_pos <- match(TRUE,!is.na(str_locate(page_lines,"explaining concepts")))
      concepts_line<- str_split(page_lines[concepts_pos]," ")[[1]]
      database$concepts[page]<- concepts_line[length(concepts_line)]
      
      
      interest_pos <- match(TRUE,!is.na(str_locate(page_lines,"stimulate interest")))
      ...
      availability_pos <- match(TRUE,!is.na(str_locate(page_lines,"availability")))
...
output_database[,5:14] <- output_database[,5:14]%>%
  mutate_all(destring)
