...
setwd("C:/Users/thecs/Dropbox/boston_university/10-DeptRA/evaluations/input")
...
extract_information <- function(){
  ...
  pdf_text <- pdftools::pdf_text(file_to_scan)
...
file <-         "./input/fall19Evaluations.pdf"
file_to_scan <- get_file_name(file)
pdf_text <- pdftools::pdf_text(file_to_scan)
...
semester <- "fall19"
semester_to_scan <- get_semester_to_scan(semester)
...
extract_page_text <- function(page, file,semester_to_scan, database, pdf_text){
  ...
  last_name <- extract_data_point(semester_to_scan,page_lines,3)
  
  #I start by extracting the professor name. Only the pages with the numbers have professor name.
  name_pos <- match(TRUE,!is.na(str_locate(page_lines,semester_to_scan)))
...
extract_data_point <- function(string_to_identify,where_to,pos_to_return){
...
for (page in 1:total_page) {
    ...
    page_name <-  paste("./input/fall19Evaluations_",page,".png",sep="")
    ...
    if(!is.na(name_pos)){  
      ...
      database$enrolles[page]<- enrollment_line[enrollment_index]
